...
def query_by_id(id_value, id_field, fields, collection):
    ...
    if not results:
        ...
        for r in like_results:
            print(
                f"[DEBUG] Similar key: {repr(r[id_field])}, length: {len(r[id_field])}"
            )
    return results
...
def llm_summarize(results, query, llm_model="gpt-3.5-turbo"):
    ...
    response = openai.ChatCompletion.create(
        model=llm_model, messages=[{"role": "user", "content": prompt}], max_tokens=512
    )
...
def main():
    ...
    parser.add_argument(
        "--fields", type=str, default="concept_name", help="输出字段,逗号分隔"
    )
    ...
    parser.add_argument(
        "--llm", action="store_true", help="用 LLM 对检索结果做摘要/问答"
    )
    parser.add_argument(
        "--collection",
        type=str,
