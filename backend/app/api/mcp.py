...
def stop_mcp():
    ...
    try:
        ...
        return {"success": success, "running": False if success else True}
    except Exception as e:
        logger.error(f"Error stopping MCP server: {e}")
        return {"success": False, "running": True, "error": str(e)}
...
def set_data(request: VerseMindDataRequest):
    ...
    try:
        if request.title is None and request.reference is None:
            raise HTTPException(
                status_code=400, detail="Either title or reference must be provided"
            )
        ...
        reference = (
            ...
            else current_data.get("reference")
        )
        ...
        return {"success": True, "title": title, "reference": reference}
