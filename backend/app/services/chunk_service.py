...
class ChunkService:
    ...
    def __init__(self, documents_dir=None, chunks_dir=None):
        ...
        self.documents_dir = documents_dir or str(
            project_root / "backend" / "01-loaded_docs"
        )
        self.chunks_dir = chunks_dir or str(
            project_root / "backend" / "02-chunked-docs"
        )
        ...
        self.logger.debug(
            f"ChunkService initialized with documents_dir={self.documents_dir}, chunks_dir={self.chunks_dir}"
        )
    ...
    def _find_document_path(self, document_id: str) -> str:
        ...
        try:
            document_path = (
                self._find_document_for_tests(document_id)
                if self._is_test_environment()
                else self._find_document(document_id)
            )
            ...
            self.logger.debug(
                f"Document size: {os.path.getsize(document_path) if os.path.exists(document_path) else 'N/A'}"
            )
    ...
    def _apply_chunking_strategy(
        self, strategy: str, text_content: str, chunk_size: int, overlap: int
    ) -> List[Dict[str, Any]]:
        """
        根据策略对文本内容进行分块
        ...
        try:
            ...
            if len(parts) >= 3:
                ...
                if (
                    len(parts[-2]) == 15
                    and parts[-2][8] == "_"
                    and parts[-2][:8].isdigit()
                    and parts[-2][9:].isdigit()
                ):
                    # 倒数第三个部分以前为原始文件名
    ...
    def _save_chunk_result(
        ...
        if document_name:
            result_file = (
                f"{document_name}_{document_id}_{timestamp}{CHUNKS_JSON_SUFFIX}"
            )
        else:
    ...
    def create_chunks(
        ...
        """
        self.logger.debug(
            f"Received request to chunk document_id: '{document_id}', strategy: '{strategy}'"
        )
        ...
        if file_ext == ".csv":
            self.logger.debug(
                f"[ChunkService] Chunking CSV rows for document: {document_id}"
            )
            import csv
            ...
            chunks = self._apply_chunking_strategy(
                strategy, text_content, chunk_size, overlap
            )
        ...
        # 保存分块结果
        result_file = self._save_chunk_result(
            result, document_name, document_id, timestamp
        )
    ...
    def _should_process_chunk_file(
        self, filename: str, document_id_filter: str
    ) -> bool:
        """判断是否应该处理该分块文件"""
        # 文件必须是分块结果文件
    ...
    def _extract_chunk_metadata(
        self, chunk_data: Dict[str, Any], filename: str, file_path: str
    ) -> Dict[str, Any]:
        """从分块数据中提取元数据信息"""
        ...
        metadata = {
            "id": chunk_data.get(
                "chunk_id"
            ),  # This is the ID of the chunking operation
            "file": filename,
            "path": file_path,
            ...
            "document_id": chunk_data.get(
                "document_id"
            ),  # Ensure this is included for filename lookup
            "document_name": document_name,  # Use the document name we extracted
        }

        self.logger.debug(
            f"[_extract_chunk_metadata] Extracted metadata: id={metadata['id']}, document_id={metadata['document_id']}, document_name={metadata['document_name']}"
        )
        return metadata
    ...
    def _process_chunk_files(
        self, all_files: List[str], document_id_filter: str
    ) -> List[Dict[str, Any]]:
        """处理文件夹中的分块文件
        ...
        for filename in all_files:
            should_process = self._should_process_chunk_file(
                filename, document_id_filter
            )
            self.logger.debug(
                f"Checking file: {filename}, should process: {should_process}"
            )
            ...
            if (
                document_id_filter
                and document_id_filter not in filename
                and chunk_data.get("document_id") != document_id_filter
            ):
                self.logger.debug(
                    f"Document ID mismatch. Filter: '{document_id_filter}', File has: '{chunk_data.get('document_id')}'"
                )
                continue

            chunk_metadata = self._extract_chunk_metadata(
                chunk_data, filename, file_path
            )
            chunk_files.append(chunk_metadata)
            self.logger.debug(
                f"Added chunk: {chunk_metadata['id']} from file: {filename}"
            )
    ...
    def get_document_chunks(self, document_id_filter: str) -> List[Dict[str, Any]]:
        """获取指定文档的所有分块结果文件信息"""
        self.logger.debug(
            f"Looking for chunks with document_id_filter: '{document_id_filter}'"
        )
        ...
        if not os.path.exists(self.chunks_dir):
            ...
            self.logger.warning(
                f"Absolute path of missing chunks directory: {abs_path}"
            )
            return []
        ...
        if not document_id_filter or document_id_filter.lower() == "list":
            self.logger.debug(
                "Empty filter or 'list' command detected - retrieving all chunk files"
            )
            chunk_files = self._process_all_chunk_files(all_files)
            self.logger.debug(
                f"Returning all {len(chunk_files)} chunks without filtering"
            )
            return chunk_files
        ...
        self.logger.debug(
            f"Returning {len(chunk_files)} chunks for document_id: {document_id_filter}"
        )
        return chunk_files
    ...
    def delete_chunk_result(self, chunk_id: str) -> str:
        ...
        if found_file:
            ...
            raise FileNotFoundError(
                f"Chunk result file containing chunk_id '{chunk_id}' not found."
            )
    ...
    def _check_chunk_id_match(
        self, chunk_data: Dict[str, Any], chunk_id: str
                ...
                for temp_filename in os.listdir(temp_dir):
                    if temp_filename.endswith(".pdf") and document_id in temp_filename:
                        ...
                        if self._check_temp_file(temp_filepath):
                            self.logger.info(
                                f"Found temporary test file: '{temp_filepath}'"
                            )
    ...
    def _find_metadata_file(self, document_id: str, metadata_dir: str) -> Optional[str]:
        ...
        self.logger.error(
            f"No metadata JSON file found in '{metadata_dir}' starting with '{document_id}'"
        )
        return None

    def _check_direct_document_path(
        self, document_id: str, workspace_root: str
    ) -> Optional[str]:
        """检查document_id是否直接引用storage/documents中的文件"""
        for ext in [".pdf", ".txt", ".md", DOCX_EXTENSION]:
            potential_path = os.path.join(
                workspace_root, "storage", "documents", document_id + ext
            )
            if os.path.exists(potential_path):
                self.logger.info(
                    f"Found document directly in storage: {potential_path}"
                )
    ...
    def _load_metadata_json(self, json_path: str) -> Optional[dict]:
        ...
        try:
            ...
            self.logger.error(
                f"Error loading or parsing metadata JSON '{json_path}': {e}"
            )
            return None

    def _extract_path_from_metadata(
        self, metadata: dict, json_path: str
    ) -> Optional[str]:
        """从元数据中提取内容文件路径"""
        potential_path_keys = [
        ...
        self.logger.warning(
            f"Preferred path keys not found in metadata JSON '{json_path}'"
        )
        return None

    def _try_build_path_with_extension(
        self, document_id: str, metadata: dict, workspace_root: str
    ) -> Optional[str]:
        """尝试使用文档ID和扩展名构建路径"""
        # 尝试从元数据中获取原始扩展名
        ...
        if original_extension:
            ...
            potential_path = os.path.join(
                workspace_root, "storage", "documents", document_id + original_extension
            )
            if os.path.exists(potential_path):
                self.logger.info(
                    f"Found path using extension from metadata: '{potential_path}'"
                )
                return potential_path
        ...
        for ext in possible_extensions:
            ...
            if os.path.exists(potential_path):
                self.logger.info(
                    f"Found file with extension '{ext}': '{potential_path}'"
                )
                return potential_path
    ...
    def _resolve_content_path(
        self, path: str, workspace_root: str, json_path: str = None
    ) -> Optional[str]:
        """解析内容文件的绝对路径"""
        ...
        if not os.path.exists(resolved_path) and json_path:
            resolved_path = self._try_path_relative_to_json(
                path, json_path, resolved_path
            )
    ...
    def _try_path_relative_to_json(
        self, path: str, json_path: str, current_resolved_path: str
    ) -> str:
        ...
        # 尝试相对于JSON文件的位置
        path_relative_to_json = os.path.normpath(
            os.path.join(os.path.dirname(json_path), path)
        )
    ...
    def _find_document(self, document_id: str) -> Optional[str]:
        ...
        """
        self.logger.info(
            f"Attempting to find content file for document_id: '{document_id}'"
        )
            ...
            for filename in os.listdir(storage_dir):
                if document_id in filename:
                    ...
                    if os.path.isfile(full_path):
                        self.logger.info(
                            f"Found document with ID in filename: {full_path}"
                        )
                        return full_path
    ...
    def _process_metadata(
        self, metadata_json_path: str, document_id: str, workspace_root: str
    ) -> Optional[str]:
        ...
        if not content_path:
            content_path = self._try_build_path_with_extension(
                document_id, metadata, workspace_root
            )
            if not content_path:
                return None

        # Resolve the final content path
        return self._resolve_content_path(
            content_path, workspace_root, metadata_json_path
        )
    ...
    def _extract_page_text(self, page, page_num) -> str:
        ...
        try:
            ...
            self.logger.warning(
                f"HTML extraction failed for page {page_num}: {e_html}. Falling back to text."
            )
    ...
    def _chunk_by_langchain_recursive(
        self, text: str, chunk_size: int, overlap: int
    ) -> List[Dict[str, Any]]:
    ...
    def _append_line_to_section(
        self, current_section: Dict[str, str], line: str
    ) -> None:
        """将行添加到当前章节内容"""
    ...
    def _chunk_by_heading(self, text: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            if self._is_heading(line):
                # 处理当前章节
                self._process_current_section(
                    sections, current_section, start_pos, current_position
                )
    ...
    def _is_test_environment(self) -> bool:
        ...
        return (
            "PYTEST_CURRENT_TEST" in os.environ or os.environ.get("TEST_ENV") == "true"
        )
    ...
    def _find_exact_match_in_temp(
        self, document_id: str, temp_dir: str
    ) -> Optional[str]:
        ...
        for temp_filename in os.listdir(temp_dir):
            if temp_filename.endswith(".pdf") and document_id in temp_filename:
                ...
                if os.path.exists(temp_filepath):
                    self.logger.info(
                        f"Found test file with matching ID: '{temp_filepath}'"
                    )
                    return temp_filepath
        return None
