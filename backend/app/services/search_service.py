...
class SearchService:
    ...
    def __init__(
        self,
        indices_dir=os.path.join("storage", "indices"),
    ...
    def _initialize_search_info(
        self,
        index_id_or_collection: str,
        query: str,
        top_k: int,
    ...
    def _find_and_validate_index_files(
        self, index_id_or_collection: str, search_info: Dict[str, Any]
    ) -> List[str]:
        """查找并验证索引文件"""
        self.logger.debug(
            f"Looking for indices with ID or collection={index_id_or_collection}"
        )
        ...
        self.logger.debug(
            f"Found {len(index_files)} indices for collection or ID: {index_id_or_collection}"
        )
    ...
    def _extract_filename_from_document_id(self, doc_id: str) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"Error extracting filename from document_id '{doc_id}': {str(e)}"
            )

        return ""

    def _load_index_data(
        self, index_file: str, search_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        """加载索引数据并提取基本信息"""
        ...
        if not document_filename and "document_id" in index_data:
            document_filename = self._extract_filename_from_document_id(
                index_data["document_id"]
            )
            if document_filename:
                self.logger.debug(
                    f"Extracted document_filename '{document_filename}' from document_id '{index_data['document_id']}'"
                )
        ...
        self.logger.debug(
            f"Found index for document_id={document_id}, document_filename={document_filename}, vector_db={vector_db}"
        )

        return index_data

    def _extract_provider_from_embedding_file_name(
        self, file_name: str, known_providers: List[str], embedding_model_str: str
    ) -> Tuple[str, str, bool]:
        """从嵌入文件名中提取提供商和模型信息"""
        provider = ""
        ...
        for p in known_providers:
            if f"_{p}_" in file_name:
                ...
                self.logger.debug(
                    f"Extracted provider='{provider}' and model='{model}' from file name"
                )
                break

        return provider, model, provider_found

    def _extract_provider_from_embedding_file_content(
        self, embedding_file_path: str
    ) -> Tuple[str, str, bool]:
        """从嵌入文件内容中提取提供商和模型信息"""
        provider = ""
        model = ""
        ...
        try:
            with open(embedding_file_path, "r", encoding="utf-8") as f:
                ...
                if "provider" in embed_data:
                    ...
                    self.logger.debug(
                        f"Extracted provider='{provider}' and model='{model}' from embedding file content"
                    )
                    provider_found = True
        except Exception as e:
    ...
    def _extract_provider_from_model_list(
        self, embedding_model_str: str
    ) -> Tuple[str, str]:
        """通过可用模型列表匹配提供商和模型"""
        provider = ""
        model = embedding_model_str
        ...
        if embedding_model_str.startswith("bge-"):
            provider = "ollama"
            self.logger.debug(
                f"Special handling: BGE model '{model}' assigned to provider '{provider}'"
            )
                ...
                for provider_name, provider_models in models_info["providers"].items():
                    for model_info in provider_models:
                        if model_info.get("id") == embedding_model_str:
                            ...
                            self.logger.debug(
                                f"Found model '{model}' in provider '{provider}' model list"
                            )
                            return provider, model
        except Exception as e:
    ...
    def _extract_provider_from_model_name(
        self, embedding_model_str: str, known_providers: List[str]
    ) -> Tuple[str, str]:
        """从模型名称中推断提供商"""
        provider = ""
        model = embedding_model_str
        ...
        for p in known_providers:
            if embedding_model_str.startswith(f"{p}-"):
                ...
                self.logger.debug(
                    f"Extracted provider='{provider}' from model name prefix"
                )
                return provider, model
        ...
        if embedding_model_str.startswith("bge-"):
            ...
            self.logger.debug(
                f"Special handling for BGE model: Using provider='{provider}' with model='{model}'"
            )
            return provider, model
        ...
        self.logger.debug(
            f"No provider identified. Using provider='{provider}' with model='{model}'"
        )
        return provider, model

    def _extract_provider_and_model(
        self, embedding_model_str: str, document_id: str, embedding_id: str
    ) -> Tuple[str, str]:
        ...
        try:
            ...
            if embedding_file_path:
                self.logger.debug(
                    f"Using embedding file path to detect provider: {embedding_file_path}"
                )
                file_name = os.path.basename(embedding_file_path)

                # 从文件名中提取提供商信息
                provider, model, provider_found = (
                    self._extract_provider_from_embedding_file_name(
                ...
                provider, model, provider_found = (
                    self._extract_provider_from_embedding_file_content(
                        embedding_file_path
                    )
                )
            ...
            # 通过可用模型列表匹配
            provider, model = self._extract_provider_from_model_list(
                embedding_model_str
            )
            if provider:
                return provider, model
            ...
            # 3. 从模型名称中推断
            return self._extract_provider_from_model_name(
                embedding_model_str, known_providers
            )
    ...
    def _perform_single_index_search(
        self,
        current_index_file: str,
        ...
        try:
            ...
            if not doc_filename and "document_id" in current_index_data:
                doc_filename = self._extract_filename_from_document_id(
                    current_index_data["document_id"]
                )

            # 更新集合信息
            self._update_collection_info(
                collection_info, doc_id, vec_db, embed_model, doc_filename
            )

            # 执行向量搜索
            self.logger.debug(
                f"Searching index: {os.path.basename(current_index_file)}"
            )
            ...
            self.logger.debug(
                f"Found {len(index_search_results)} results in index {os.path.basename(current_index_file)}"
            )
    ...
    def _prepare_query_vector(
        self, search_info: Dict[str, Any], index_data: Dict[str, Any], query: str
    ) -> List[float]:
        ...
        self.logger.debug(
            f"Using provider='{provider}' and model='{model}' for query vector generation"
        )
    ...
    def _calculate_search_stats(
        self, search_results: List[Dict[str, Any]], search_info: Dict[str, Any]
    ) -> None:
        """计算搜索结果的统计信息"""
        ...
        if search_results:
            search_info["similarity_range"] = {
                ...
                "avg": sum(r["similarity"] for r in search_results)
                / len(search_results),
            }

    def _extract_document_filename_from_results(
        self, search_results: List[Dict[str, Any]]
    ) -> Optional[str]:
        """从搜索结果中提取文档文件名"""
        try:
            for result_item in search_results:
                if result_item.get("metadata") and result_item["metadata"].get(
                    "source"
                ):
                    ...
                    if isinstance(source_path, str):
                        ...
                        if filename:
                            self.logger.debug(
                                f"Successfully extracted filename: {filename} from source path"
                            )
                            return filename
        except Exception as e:
            self.logger.error(
                f"Error extracting document filename from results: {str(e)}"
            )
        return None

    def _extract_filename_from_document_id_parts(
            ...
            if isinstance(document_id, str):
                ...
                if len(parts) >= 3:
                    ...
                    if (
                        ...
                        self.logger.debug(
                            f"Extracted document name from ID: {clean_name}"
                        )
                        return clean_name
        except Exception as e:
        ...
        if len(parts) >= 4:
            ...
            try:
                if (
                    ...
                    self.logger.debug(
                        f"Cleaned document_filename to: {document_filename}"
                    )
            except Exception as e:
                self.logger.debug(f"Error while cleaning document filename: {str(e)}")

        # 为中文文档添加前缀
        if document_filename and (
            "中文" in document_filename
            or "的" in document_filename
            or "一" in document_filename
        ):
    ...
    def _generate_collection_display_name(
        self, collection_info: Dict[str, Any], original_id_or_collection: str
    ) -> str:
        """生成集合显示名称"""
        ...
        if collection_display_name:
            ...
            # 对中文文档进行特殊处理
            if (
                "中文" in collection_display_name
                or "的" in collection_display_name
                or "一" in collection_display_name
            ):
                # 这可能是一个中文文档名，给它添加一个前缀以便更容易识别
                if not collection_display_name.startswith(
                    "文档:"
                ) and not collection_display_name.startswith("集合:"):
                    collection_display_name = f"文档: {collection_display_name}"

        return collection_display_name

    def _save_search_results(
        self, result: Dict[str, Any], search_id: str, timestamp: str
    ) -> str:
        """保存搜索结果到文件"""
    ...
    def search(
        ...
        """
        self.logger.debug(
            f"Starting search with index_id_or_collection={index_id_or_collection}, query={query}, top_k={top_k}, similarity_threshold={similarity_threshold}"
        )
        ...
        search_results, index_files, collection_info = self._execute_search_process(
            index_id_or_collection,
            original_id_or_collection,
            query,
            top_k,
            similarity_threshold,
            min_chars,
    ...
    def _execute_search_process(
        self,
        index_id_or_collection: str,
        original_id_or_collection: str,
        query: str,
        top_k: int,
        similarity_threshold: float,
        min_chars: int,
        ...
        # 查找索引文件
        index_files = self._find_and_validate_index_files(
            index_id_or_collection, search_info
        )
    ...
    def _prepare_index_data(
        self, index_files: List[str], search_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        """准备索引数据，从第一个索引文件中加载"""
    ...
    def _execute_vector_search(
        ...
        self.logger.debug(
            f"Performing vector search with {len(query_vector)}-dimensional query vector"
        )
    ...
    def _process_document_metadata(
        self, search_info: Dict[str, Any], search_results: List[Dict[str, Any]]
    ) -> Tuple[str, str]:
        """处理文档元数据，确保有有效的文件名"""
        document_filename = search_info.get("document_filename", "")
        document_id = search_info.get("document_id", "")
        self.logger.debug(
            f"Initial document_filename: {document_filename}, document_id: {document_id}"
        )
    ...
    def _extract_document_filename_from_sources(
        self,
        document_id: str,
        ...
        # 2. 从搜索结果中提取
        extracted_filename = self._extract_document_filename_from_results(
            search_results
        )
    ...
    def _build_result_object(
        ...
        # 添加便于访问的辅助字段
        result["search_info"]["document_filename"] = (
            document_filename if len(index_files) == 1 else collection_display_name
        )
    ...
    def _save_and_log_results(
        self, result: Dict[str, Any], search_results: List[Dict[str, Any]]
    ) -> None:
        """保存搜索结果并记录日志"""
        # 保存搜索结果
        result_file = self._save_search_results(
            result, result["search_id"], result["timestamp"]
        )
        ...
        if search_results:
            ...
            self.logger.debug(
                f"Found {len(search_results)} results with similarities: {similarities}"
            )
        else:
    ...
    def _get_search_directories(self) -> List[str]:
        ...
        vector_db_dir = (
            ...
            if hasattr(settings, "VECTOR_STORE_PERSIST_DIR")
            else os.path.join(self.storage_dir, "storage", "vector_db")
        )
        return [self.indices_dir, vector_db_dir]

    def _find_matching_index_in_directory(
        self, dir_path: str, index_id: str
    ) -> Optional[str]:
        """在指定目录中查找匹配的索引文件"""
        ...
        self.logger.debug(
            f"No index file with index_id='{index_id}' found in {dir_path}"
        )
        return None
    ...
    def _safely_read_json_file(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(
                f"Could not decode JSON from file: '{os.path.basename(file_path)}'"
            )
        except Exception as e:
            self.logger.error(
    ...
    def _is_index_match(
        self, index_data: Dict[str, Any], filename: str, index_id: str
    ) -> bool:
        """检查索引数据是否与目标索引ID匹配"""
        # 检查JSON中的索引ID
        ...
        if internal_index_id == index_id:
            self.logger.debug(
                f"Match found: File '{filename}' contains index_id='{index_id}'"
            )
            return True
        ...
        if index_id in filename:
            self.logger.debug(
                f"Match found by filename: '{filename}' contains index_id='{index_id}'"
            )
            return True

        return False

    def _find_index_files_by_collection_or_id(
        self, index_id_or_collection: str
    ) -> List[str]:
        """
        查找指定ID或集合名称的索引文件
        ...
        """
        self.logger.debug(
            f"Searching for index files with index_id or collection_name='{index_id_or_collection}'"
        )
        ...
        # 在所有目录中搜索匹配的索引文件
        matching_files = self._find_all_matching_indices(
            possible_dirs, index_id_or_collection
        )
        ...
        self.logger.debug(
            f"Found {len(matching_files)} matching index files for '{index_id_or_collection}'"
        )
        return matching_files

    def _find_all_matching_indices(
        self, directories: List[str], index_id_or_collection: str
    ) -> List[str]:
        """在多个目录中查找所有匹配的索引文件"""
        matching_files = []
        ...
        for dir_path in directories:
            ...
            matching_files.extend(
                self._find_matches_in_directory(dir_path, index_id_or_collection)
            )

        return matching_files

    def _find_matches_in_directory(
        self, dir_path: str, index_id_or_collection: str
    ) -> List[str]:
        """在单个目录中查找匹配的索引文件"""
        matches = []
    ...
    def _is_index_match(
        self, index_data: Dict, filename: str, index_id_or_collection: str
    ) -> bool:
        """检查索引数据是否与搜索条件匹配"""
        # 检查索引ID
        internal_index_id = index_data.get("index_id")
        if internal_index_id == index_id_or_collection:
            self.logger.debug(
                f"Match found by index_id: File '{filename}' contains index_id='{index_id_or_collection}'"
            )
            return True
        ...
        if collection_name == index_id_or_collection:
            self.logger.debug(
                f"Match found by collection_name: File '{filename}' belongs to collection='{index_id_or_collection}'"
            )
            return True
        ...
        if index_id_or_collection in filename:
            self.logger.debug(
                f"Match found by filename: '{filename}' contains '{index_id_or_collection}'"
            )
            return True
    ...
    def _try_single_index_fallback(self, index_id_or_collection: str) -> List[str]:
        ...
        self.logger.debug(
            f"No matching collection found, trying single index fallback for '{index_id_or_collection}'"
        )
    ...
    def _find_embedding_file(
        self, document_id: str, embedding_id: str = None
    ) -> Optional[str]:
        """查找指定文档的嵌入文件"""
        self.logger.debug(
            f"Searching for embedding file with document_id='{document_id}' and embedding_id='{embedding_id}'"
        )
        ...
        for file_path, filename in potential_files:
            embedding_file = self._check_embedding_file_match(
                file_path, filename, embedding_id
            )
            if embedding_file:
    ...
    def _get_potential_embedding_files(
        self, dir_path: str, document_id: str
    ) -> List[Tuple[str, str]]:
        """获取可能包含指定文档ID的所有文件"""
        potential_files = []
    ...
    def _check_embedding_file_match(
        self, file_path: str, filename: str, embedding_id: str = None
    ) -> Optional[str]:
        """检查文件是否为所需的嵌入文件"""
    ...
    def _check_embedding_id_match(
        self, data: Dict, filename: str, embedding_id: str = None
    ) -> bool:
        """检查嵌入ID是否匹配"""
        if not embedding_id:
            return True
        ...
        if internal_embedding_id and internal_embedding_id != embedding_id:
            self.logger.debug(
                f"File '{filename}' has embedding_id='{internal_embedding_id}' which doesn't match target '{embedding_id}'"
            )
            return False
    ...
    def _has_embedding_keys(self, data: Dict, filename: str) -> bool:
        ...
        for key in ["embeddings", "vectors", "vector"]:
            if key in data:
                self.logger.debug(
                    f"Match found: File '{filename}' contains '{key}' key"
                )
                return True
        return False
    ...
    def _generate_query_vector(
        self, query: str, provider: str, model: str
    ) -> List[float]:
        """
        生成查询向量
        ...
        if "bge" in model.lower() and provider != "ollama":
            self.logger.debug(
                f"Converting BGE model provider from '{provider}' to 'ollama'"
            )
            provider = "ollama"

        self.logger.debug(
            f"Generating query vector with provider={provider}, model={model}"
        )
        ...
        try:
            ...
            if "bge" in model.lower() and provider != "ollama":
                ...
                try:
                    ...
                    embed_service = EmbedService()
                    vector = embed_service.generate_embedding_vector(
                        query, "ollama", model
                    )
                    self.logger.debug(
                        f"Successfully generated vector with ollama provider, dimensions: {len(vector)}"
                    )
                    return vector
    ...
    def _vector_search_from_index(
        ...
        """
        self.logger.debug(
            f"Performing vector search with threshold={similarity_threshold}, top_k={top_k}"
        )
        ...
        try:
            ...
            if not embedding_file:
                self.logger.error(
                    f"Embedding file not found for document ID: {document_id}"
                )
            ...
            self.logger.debug(
                f"Loaded {len(embeddings)} embeddings from {embedding_file}"
            )
            ...
            for item in embeddings:
                ...
                if similarity >= similarity_threshold and len(text) >= min_chars:
                    results.append(
                        {"text": text, "similarity": similarity, "metadata": metadata}
                    )
            ...
            self.logger.debug(
                f"Found {len(results)} results after filtering by threshold and length"
            )
            return results
